...
run inside of. Phase 8 (`internal/sql`) reaches this exact model
through SQL syntax rather than new mechanism — see §11 for the
resulting implementation decisions and [`docs/sql.md`](sql.md) for the
SQL-facing semantics.
...
## 11. Phase 8 implementation decisions (resolved)

- **SQL's `BEGIN`/`COMMIT`/`ROLLBACK` are this document's §1 lifecycle,
  reached through SQL syntax, not a second transaction model.**
  `internal/sql.Session` (`docs/sql.md` §5.4) tracks at most one open
  explicit transaction, backed by exactly one `Txn` (standalone:
  `internal/txn.Txn`; replicated: a session-equivalent built on
  `internal/node.Node.Propose`/`BeginReadIndex`, `internal/sql/engine.go`).
  Every statement between SQL `BEGIN` and `COMMIT` calls that same
  `Txn`'s `Write`/`Delete`/`Read` — accumulating into the identical
  single local write set §1-§2 describe — and SQL `COMMIT` calls that
  `Txn`'s `Commit` exactly once, submitting the whole accumulated set
  as **one** `CommitTxn` command (§3), never one per statement. SQL
  `ROLLBACK` calls `Abort` (§2's "no durable trace, no RequestID
  outcome recorded"). A statement executed with no open explicit
  transaction is its own single-statement `Txn`, begun and committed
  by that one call — the auto-commit case this document's examples
  throughout already assume for a "normal" transaction.
- **A statement error inside an open explicit SQL transaction aborts
  the whole transaction**, not just the failing statement — a
  narrower policy than this document requires (nothing here forbids a
  more forgiving "the transaction stays open, retry the failed
  statement" model), chosen for simplicity and determinism at the SQL
  layer specifically; the underlying `Txn`/`Manager`/`Node` API this
  document specifies does not itself impose this — a future, more
  permissive SQL-level policy would not require any change below
  `internal/sql`.
- **`RequestID` for SQL is a parameter to `Execute`/`ExecuteStatement`,
  never derived from SQL text and never generated by ChronicleDB**
  (§6, §3's "opaque, client-supplied idempotency key" — unchanged by
  SQL's existence). It attaches to whichever statement actually
  triggers a commit (an auto-commit statement, or an explicit
  `COMMIT`) and is not consulted otherwise — in particular, never for
  a pure `SELECT`, which takes this document's §9/§10 read-only bypass
  exactly like any other empty-mutation commit.
- **A genuine SQL-level retry must be checked for *after* `Begin`, not
  before.** `internal/sql.Session.executeDML`'s idempotency
  short-circuit (`docs/sql.md` §5.3) originally checked
  `RequestID`'s recorded outcome before beginning a fresh `Txn` for an
  auto-commit statement — a real race against a replicated `Engine`
  still catching up: the outcome lookup could miss even though the row
  it protects is *about to* become visible from underneath the same
  statement, since `Begin`'s `ReadIndex` round trip (docs/replication.md
  §4) is precisely what guarantees this node has caught up to at least
  that outcome's own commit point. Checking after `Begin` closes the
  race: by then, if the original commit's `CommitSeq` is at or before
  this `Txn`'s `StartSeq` (which it always is, for a genuine retry),
  the outcome is guaranteed already visible.
